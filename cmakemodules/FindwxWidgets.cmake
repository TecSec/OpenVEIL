...
if(wxWidgets_FIND_STYLE STREQUAL "win32")
    ...
    if(WX_USE_REL_AND_DBG)
      foreach(LIB ${${_LIBS}})
        ...
        if(WX_${LIB} AND WX_${LIB}d)
          ...
          list(APPEND wxWidgets_LIBRARIES_DEBUG
            ${WX_${LIB}d}
            )
        ...
        if(${TS_X_PLATFORM} STREQUAL "x64")
          ...
          string(REPLACE ".lib" "_${TS_TOOLSET}0_tecsec.dll" __tmp "${WX_${LIB}d}")
			...
			string(REPLACE "wxmsw31u" "wxmsw312u" __tmp "${__tmp}")
			string(REPLACE "wxbase31u" "wxbase312u" __tmp "${__tmp}")
  ...
  if(WX_ROOT_DIR)
    ...
    elseif(CMAKE_CL_64)
      set(WX_LIB_DIR_PREFIX vc${TS_TOOLSET_NUMBER}_x64)
    else()
      set(WX_LIB_DIR_PREFIX vc${TS_TOOLSET_NUMBER})
    endif()
  ...
  if(wxWidgets_FOUND)
    if(NOT TARGET WxWidgets)
		if(WIN32)
		  ...
		  set_property(TARGET WxWidgets PROPERTY IMPORTED_LOCATION_DEBUG "${wxWidgets_LIBRARIES_DEBUG}")
		  set_property(TARGET WxWidgets PROPERTY IMPORTED_LOCATION_RELEASE "${wxWidgets_LIBRARIES_RELEASE}")
		  set_property(TARGET WxWidgets PROPERTY INTERFACE_BIN_MODULES_DEBUG "${wxWidgets_DLLS_DEBUG}")
		  set_property(TARGET WxWidgets PROPERTY INTERFACE_BIN_MODULES_RELEASE "${wxWidgets_DLLS_RELEASE}")
		  set_property(TARGET WxWidgets PROPERTY IMPORTED_IMPLIB_DEBUG "${wxWidgets_LIBRARIES_DEBUG}")
		  set_property(TARGET WxWidgets PROPERTY IMPORTED_IMPLIB_RELEASE "${wxWidgets_LIBRARIES_RELEASE}")
		  set_property(TARGET WxWidgets PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${wxWidgets_INCLUDE_DIRS}")
		else(WIN32)
		  add_library(WxWidgets SHARED IMPORTED)
		  set_target_properties(WxWidgets PROPERTIES
			IMPORTED_LOCATION_DEBUG "${wxWidgets_LIBRARIES_DEBUG}"
			IMPORTED_LOCATION_RELEASE "${wxWidgets_LIBRARIES_RELEASE}"
